                ...
                for (int32_t i = 1; i <= extraAttacks; ++i) {
                    g_dispatcher().scheduleEvent(
                        ...
                        [caster, target, combatType, combatOrigin, distEffect, weaponRange]() {
                            ...
                            if (currentDist > weaponRange) return;
                            ...
                            if (shootEffect != CONST_ANI_NONE) {
                                g_game().addDistanceEffect(currentCasterPos,
                                    targetPos, shootEffect);
                            }
                            ...
                            Combat::doCombatHealth(caster, target, currentCasterPos,
                                                   extraDamage, extraParams);

                            if (currentAmmo && g_configManager().getBoolean(REMOVE_WEAPON_AMMO)) {
                                const uint16_t ammoCount = currentAmmo->getItemCount();
                                if (ammoCount > 1) {
                                    g_game().transformItem(currentAmmo,
                                        currentAmmo->getID(), ammoCount - 1);
                                } else if (ammoCount == 1) {
