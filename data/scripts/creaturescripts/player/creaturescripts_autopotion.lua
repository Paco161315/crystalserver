...
local POTION_DATA = {
    ...
    [236]   = { health = {250, 350},      flask = 283, level = 50, voc = {
        VOCATION.BASE_ID.PALADIN, VOCATION.BASE_ID.KNIGHT, VOCATION.BASE_ID.MONK
    }},
    ...
    [238]   = { mana   = {150, 250},      flask = 284, level = 80, voc = {
        VOCATION.BASE_ID.SORCERER, VOCATION.BASE_ID.DRUID,
        VOCATION.BASE_ID.PALADIN,  VOCATION.BASE_ID.MONK
    }},
...
local function canUsePotion(player, potion)
    if not player or not player:isPlayer() or not potion then return false end
    ...
    if potion.voc then
        ...
        if not table.contains(potion.voc, base) then return false end
    end

    if potion.level and player:getLevel() < potion.level then return false end
...
local function findPotion(player)
    ...
    if maxHp <= 0 or maxMana <= 0 then return nil end
    ...
    if not needHealth and not needMana then return nil end
...
local function tryHeal(player)
    ...
    if player:getStorageValue(STORAGE) ~= 1 then return end
    if player:getCondition(CONDITION_EXHAUST_HEAL) then return end
    ...
    if not potionCache[pid] then potionCache[pid] = {} end
    ...
    if not data.item or not data.item:isItem() then return end
    data.item:remove(1)
