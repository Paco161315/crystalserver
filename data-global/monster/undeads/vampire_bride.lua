...
local function checkAndTeleport(areaFromPos, areaToPos, teleportDest, spawnPos)
	...
	if killCount >= 2 then
		local spectators = Game.getSpectators(areaFromPos, false, false, 
			areaToPos.x - areaFromPos.x, areaToPos.x - areaFromPos.x,
			areaToPos.y - areaFromPos.y, areaToPos.y - areaFromPos.y)
