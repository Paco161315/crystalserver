...
local function checkAndTeleport(areaFromPos, areaToPos, teleportDest, spawnPos)
	...
	for x = areaFromPos.x, areaToPos.x do
		for y = areaFromPos.y, areaToPos.y do
			local pos = Position(x, y, areaFromPos.z)
	...
	if killCount >= 2 then
		local spectators = Game.getSpectators(areaFromPos, false, false, areaToPos.x - areaFromPos.x, areaToPos.x - areaFromPos.x, areaToPos.y - areaFromPos.y, areaToPos.y - areaFromPos.y)
